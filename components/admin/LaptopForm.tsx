...
const Field = ({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) => (
  <div>
    <label className="mb-1 block text-sm font-medium text-[#8B97A8]">{label}</label>
    {children}
    {error && <p className="mt-1 text-xs text-[#FF6B35]">{error}</p>}
  </div>
)

const inputClass = 'w-full border-[#1E2530] bg-[#161B23] text-white placeholder:text-[#8B97A8] focus:ring-[#00D4FF]'
...
export function LaptopForm({ laptop }: LaptopFormProps) {
  ...
  const CheckField = ({ name, label }: { name: keyof FormData; label: string }) => (
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Brand *" error={errors.brand?.message}>
            <Input {...register('brand')} placeholder="Dell" className={inputClass} />
          </Field>
          ...
          <Field label="CPU Generation" error={errors.cpuGeneration?.message}>
            <Input {...register('cpuGeneration')} placeholder="8" className={inputClass} />
          </Field>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Storage Type" error={errors.storageType?.message}>
            <Input {...register('storageType')} placeholder="NVMe" className={inputClass} />
          </Field>
        </div>
      ...
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Weight (kg)" error={errors.weight?.message}>
            <Input {...register('weight')} type="number" step="0.1" placeholder="1.8" className={inputClass} />
          </Field>
          <Field label="Battery Health" error={errors.batteryHealth?.message}>
            <Input {...register('batteryHealth')} placeholder="Good / 85%" className={inputClass} />
          </Field>
          <Field label="Estimated Battery Hours" error={errors.estimatedBatteryHours?.message}>
            <Input {...register('estimatedBatteryHours')} type="number" step="0.5" placeholder="6" className={inputClass} />
          </Field>
        </div>
        <div className="mt-4 flex flex-wrap gap-4">
      ...
      {/* Pricing */}
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        <h2 className="mb-4 font-semibold text-white">Pricing & Availability</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Price (NGN) *" error={errors.price?.message}>
            <Input {...register('price')} type="number" placeholder="195000" className={inputClass} />
          </Field>
          <Field label="Condition *" error={errors.condition?.message}>
            <Select onValueChange={(v) => setValue('condition', v as FormData['condition'])} defaultValue={watch('condition')}>
          ...
          <Field label="Availability" error={errors.availability?.message}>
            <Select onValueChange={(v) => setValue('availability', v as FormData['availability'])} defaultValue={watch('availability')}>
              <SelectTrigger className={inputClass}><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      {/* Scores */}
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        <h2 className="mb-4 font-semibold text-white">Performance Scores (0–100, optional)</h2>
        <div className="grid gap-4 sm:grid-cols-4">
          {['gamingScore', 'editingScore', 'programmingScore', 'batteryScore'].map((s) => (
            <Field key={s} label={s.replace('Score', ' Score').replace(/([A-Z])/g, ' $1').trim()}>
              <Input {...register(s as keyof FormData)} type="number" min="0" max="100" className={inputClass} />
            </Field>
          ))}
        </div>
        <div className="mt-4">
      ...
      <section className="rounded-xl border border-[#1E2530] bg-[#0F1318] p-6">
        ...
        {process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME ? (
          <CldUploadWidget
            uploadPreset="Maville Technologies"
            options={{ multiple: true, maxFiles: 6, resourceType: 'image' }}
            onSuccess={(result) => {
          ...
          >
            {({ open }) => (
              <button
                type="button"
                onClick={() => open()}
                className="rounded-xl border-2 border-dashed border-[#1E2530] px-6 py-8 text-center w-full text-sm text-[#8B97A8] hover:border-[#00D4FF] hover:text-[#00D4FF] transition-colors"
              >
          ...
          <p className="text-sm text-[#FFB300]">Configure Cloudinary env vars to enable image uploads</p>
        )}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {images.map((img, i) => (
              <div key={i} className="relative">
                <Image src={img} alt={`Image ${i + 1}`} width={80} height={60} className="rounded-lg object-cover" />
                <button
                  type="button"
      ...
      <div className="flex gap-3">
        <Button type="button" variant="outline" onClick={() => router.back()} className="border-[#1E2530] text-[#8B97A8]">
          Cancel
        </Button>
        <Button
          type="submit"
        ...
        >
          {isSubmitting ? 'Saving...' : laptop ? 'Update Laptop' : 'Create Laptop'}
        </Button>
      </div>
    </form>
  )
